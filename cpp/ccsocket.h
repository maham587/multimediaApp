 ...
 * - TCP/IP sockets do not preserve record boundaries but SocketBuffer solves
 * this problem.
 */
class Socket {
public:
  ...
  /// @note startup is automaticcaly called when a Socket or a ServerSocket is
  /// created
  /// @{
  static void startup();
  static void cleanup();
  ...
  /// On Unix/Linux host can be a hostname, on Windows it can only be an IP
  /// address.
  /// @return 0 on success or a negative value on error which is one of
  /// Socket::Errors
  ...
  /// @return the number of bytes that were sent, or 0 or shutdownInput() was
  /// called on the other side, or Socket::Failed (-1) if an error occured.
  ...
  /// @return the number of bytes that were received, or 0 or shutdownOutput()
  /// was called on the other side, or Socket::Failed (-1) if an error occured.
  ...
  SOCKSIZE sendTo(void const *buf, size_t len, int flags, SOCKADDR const *to,
                  socklen_t addrlen) {
    return ::sendto(sockfd_, buf, len, NO_SIGPIPE_(flags), to, addrlen);
  }

  /// Receives data from datagram socket.
  SOCKSIZE receiveFrom(void *buf, size_t len, int flags, SOCKADDR *from,
                       socklen_t *addrlen) {
...
class ServerSocket {
public:
  /// Creates a listening socket that waits for connection requests by TCP/IP
  /// clients.
  ServerSocket();

  ~ServerSocket();

  /// Accepts a new connection request and returns a socket for exchanging data
  /// with this client. This function blocks until there is a connection
  /// request.
...
/** Preserves record boundaries when exchanging messages between connected
 TCP/IP sockets.
 * Ensures that one call to readLine() corresponds to one and exactly one call
 to writeLine() on the other side.
 * By default, writeLine() adds \n at the end of each message and readLine()
 searches for \n, \r or \n\r
 * so that it can retreive the entire record.  Beware messages should thus not
 contain these charecters.
 *
 * @code
 *   int main() {
 *      Socket sock;
...
public:
  /// Constructor.
  /// _socket_ must be a connected TCP/IP Socket. It should **not** be deleted
  /// as long as the SocketBuffer is used. _inputSize_ and _ouputSize_ are the
  /// sizes of the buffers that are used internally for exchanging data.
  /// @{
   ...
   * readLine() receives one (and only one) message sent by writeLine() on the
   * other side, ie, a call to writeLine() corresponds to one and exactly one
   * call to readLine() on the other side. The received data is stored in
   * _message_. This method blocks until the message is fully received.
   *
   * @return The number of bytes that were received or one of the following
   * values:
   ...
   * writeLine() sends a message that will be received by a single call of
   * readLine() on the other side,
   *
   * @return see readLine()
   * @note if _message_ contains one or several occurences of the separator,
  ...
  /// setReadSeparator() changes the symbol used by readLine() to separate
  /// successive messages:
  /// - if _separ_ < 0 (the default) readLine() searches for \\n, \\r or \\n\\r.
  /// - if _separ_ >= 0, readLine() searches for this character to separate
  /// messages,
  /// @{
  ...
  /// setWriteSeparator() changes the character(s) used by writeLine() to
